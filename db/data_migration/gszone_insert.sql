...
insert_gwszone AS
(
	insert into afu_gewaesserschutz.gwszonen_gwszone(t_id, typ, istaltrechtlich, astatus, bemerkungen, bemerkungen_lang, geometrie)
	(
		select 
...
insert_link AS 
(
	insert into afu_gewaesserschutz.gwszonen_rechtsvorschriftgwszone(rechtsvorschrift, gwszone)
	(
		SELECT 
			tid_dok,
			tid_gwszone
...
SELECT concat_ws(' ', 'gszone_insert.sql:', count(*), 'Zeilen in [status] eingefuegt.') AS msg FROM insert_status
UNION ALL
